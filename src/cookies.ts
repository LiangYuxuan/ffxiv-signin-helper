...
export default async (): Promise<string> => {
    ...
    if (
        ...
        if (response.encrypted) {
            ...
            if (data.cookie_data['sdo.com']) {
                logger.info('使用CookieCloud中的Cookies');

                const cookies = data.cookie_data['sdo.com']
                    .filter((cookie) => cookie.domain === 'sdo.com' || cookie.domain === '.sdo.com' || cookie.domain === '.web.sdo.com' || cookie.domain === 'apiff14risingstones.web.sdo.com')
        ...
        logger.warn('CookieCloud中未找到sdo.com的Cookies');
    }
